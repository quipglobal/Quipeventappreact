...
export const HomePage: React.FC<HomePageProps> = ({ onNavigate }) => {
  ...
  const nextTier = gamificationConfig.tiers.find(
    (ti) => ti.minPoints > user.points,
  );
  ...
  const youRank = meFromLeaderboard
    ...
    : leaderboardTop3.findIndex((p) => p.points <= user.points) + 1 ||
      leaderboardTop3.length + 1;
  ...
  return (
    <div className="min-h-screen pb-6" style={{ background: t.bgPage }}>
      ...
      <div className="px-4 pt-3">
        ...
        >
          <div
            className="relative p-4 flex items-end h-full"
            style={{ minHeight: 132 }}
          >
              ...
              >
                {eventConfig?.dates && (
                  <span className="flex items-center gap-1">
                    <Calendar size={11} /> {eventConfig.dates}
                  </span>
                )}
                {eventConfig?.location && (
                  <span className="flex items-center gap-1">
                    <MapPin size={11} /> {eventConfig.location}
                  </span>
                )}
              </div>
            </div>
            {eventConfig?.logoURL && (
              <div
                className="ml-3 w-12 h-12 rounded-xl overflow-hidden flex-shrink-0"
                style={{
                  background: "#fff",
                  padding: 4,
        ...
        >
          <div className="flex items-center justify-between mb-3">
            ...
            <div
              ...
              style={{
                background: `rgba(${tierColor === "#ffd700" ? "255,215,0" : "124,58,237"},0.18)`,
          ...
          {nextTier && ptsToNextTier > 0 && (
            <>
              <div className="flex items-center justify-between mb-1.5">
                ...
                <span style={{ color: t.textMuted, fontSize: 11 }}>
                  {user.points} / {nextTier.minPoints}
                </span>
              </div>
              <div
      ...
      {nextSession && (
        <div className="px-4 pt-4">
          ...
          >
            ...
            >
              <span className="flex items-center gap-1">
                <Clock size={11} /> {nextSession.startTime} –{" "}
                {nextSession.endTime}
              </span>
              {nextSession.room && (
                <span className="flex items-center gap-1">
                  <MapPin size={11} /> {nextSession.room}
                </span>
              )}
            </div>
              ...
              >
                ...
                <span style={{ color: t.textSec, fontSize: 11 }}>
                  {nextSession.speakers
                    ...
                    .slice(0, 2)
                    .join(", ")}
                  {nextSession.speakers.length > 2
                    ? ` +${nextSession.speakers.length - 2}`
                    : ""}
                </span>
              </div>
            )}
            {nextSession.assignedAudience &&
              nextSession.assignedAudience.length > 0 && (
                <div
        ...
        <div className="pt-4">
          <div className="px-4 flex items-center justify-between mb-2">
            <h2
              ...
              style={{ color: t.text, fontSize: 14, fontWeight: 800 }}
            >
          ...
          <div className="overflow-x-auto px-4 no-scrollbar">
            <div className="flex gap-3 pb-1">
              {featuredSpeakers.map((sp, i) => (
                ...
                >
                  <div
                    className="w-14 h-14 mx-auto rounded-full overflow-hidden mb-2"
                    style={{
                      background: "linear-gradient(135deg,#7c3aed,#4f46e5)",
                    }}
                  >
      ...
      <div className="px-4 pt-4">
        ...
        >
          ...
          <div
            ...
            style={{
              background:
                "linear-gradient(90deg, rgba(245,158,11,0.12), rgba(124,58,237,0.12))",
