...
export default function ProfileScreen() {
  ...
  const { user, logout, bookmarkedSessions, completedChallenges } = useAuth();
  ...
  const tierColor = user?.tier
    ? { Bronze: '#cd7f32', Silver: '#c0c0c0', Gold: '#ffd700', Platinum: '#e5e4e2' }[user.tier] ?? colors.primary
    : colors.primary;
    ...
    >
      ...
      <LinearGradient colors={['#1a0d2e', '#0d1a2e']} style={styles.profileCard}>
        ...
        <View style={styles.roleBadge}>
          <Ionicons
            name={user?.role === 'sponsor' ? 'briefcase' : 'people'}
            size={12}
