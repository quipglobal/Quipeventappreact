...
function normalizeMeeting(raw: any, currentUserId?: string): Meeting {
  ...
  return {
    ...
    attendee: {
      ...
      userId:           String(counterparty.id ?? counterparty.user_id ?? ''),
      memberId:         String(counterparty.membership_id ?? counterparty.id ?? ''),
      ...
      interests:        [],
      interestedTopics: [],
      avatar:           counterparty.avatar_url ?? counterparty.profile_image ?? null,
      bio:              null,
      isCheckedIn:      false,
      status:           '',
      badgeCode:        null,
